...
- **CJK / non-Latin self-check before committing any long Spanish article.** Long Spanish drafts written in batches have a known failure mode where CJK characters (Chinese, Japanese, Korean) get injected into a Spanish sentence (e.g., "y很有可能" instead of "y muy probablemente"). Before `git add`, scan the file with a regex like `re.findall(r'[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff]', content)` — if any matches, fix and re-check. Run from the project root: `python -c "import re; t=open('content/<silo>/<file>.md', encoding='utf-8').read(); print(len(re.findall(r'[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff]', t)))"`. Must print `0`.
